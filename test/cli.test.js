...
// --- the canvas link, its cache, and the folder it replaces ------------------
// All three rows are structural by design: this CLI has no credentials and no network,
// so it can say the cache is intact and must never imply it is current.

const ID = 'b1a5cf4e-8121-43ec-a4b3-beb8084ec8dd';
const OTHER = '00000000-1111-2222-3333-444444444444';
const cacheFile = (dir) => path.join(dir, 'dev/flowpad/tasks-cache.md');
const writeCache = (dir, header) => fs.writeFileSync(cacheFile(dir), `${header}\n\n# Tasks\n`);
const generated = (id, rev = 42) =>
  `<!-- GENERATED — DO NOT EDIT · projectId: ${id} · revision: ${rev} · generatedAt: 2026-08-05T09:00:00Z -->`;

test('link writes the pointer, is idempotent, and keeps other keys', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  assert.match(run(dir, ['link', ID], { mustPass: true }).out, /wrote.*project\.json/);

  const file = path.join(dir, 'dev/flowpad/project.json');
  assert.deepStrictEqual(JSON.parse(fs.readFileSync(file, 'utf8')), { projectId: ID });
  assert.match(run(dir, ['link', ID], { mustPass: true }).out, /same/);

  fs.writeFileSync(file, JSON.stringify({ projectId: ID, note: 'mine' }));
  run(dir, ['link', OTHER], { mustPass: true });
...
test('link refuses an id that is not one, and reports what is linked', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  assert.match(run(dir, ['link']).out, /Not linked/);
  assert.strictEqual(run(dir, ['link', 'my-project']).code, 1);
  assert.ok(!fs.existsSync(path.join(dir, 'dev/flowpad/project.json')));
  run(dir, ['link', ID]);
  assert.match(run(dir, ['link'], { mustPass: true }).out, new RegExp(ID));
});

test('an unlinked repository is told so, but it is not a warning', () => {
  // Most repositories are not linked to anything. A tool that treats "you did not opt
  // in" as a defect trains people to ignore its amber rows.
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  const { out } = run(dir, ['check']);
  assert.match(line(out, 'project'), /INFO/);
  assert.strictEqual(line(out, 'tasks-cache'), ''); // nothing to say without an address
});

test('a linked repository with an intact cache passes, and says freshness is not checked', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  run(dir, ['link', ID]);
  writeCache(dir, generated(ID));
  const { out } = run(dir, ['check']);
  assert.match(line(out, 'project'), /PASS/);
  assert.match(line(out, 'tasks-cache'), /PASS/);
  assert.match(line(out, 'tasks-cache'), /revision 42/);
  // The green must not be readable as "up to date" — nothing here can know that.
  assert.match(line(out, 'tasks-cache'), /cannot tell you whether it is current/);
});

test('a hand-edited or foreign cache header goes amber', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  run(dir, ['link', ID]);

  writeCache(dir, '# Tasks I started editing');
  assert.match(line(run(dir, ['check']).out, 'tasks-cache'), /WARN.*hand-edited/);

  writeCache(dir, generated(OTHER));
  assert.match(line(run(dir, ['check']).out, 'tasks-cache'), /WARN.*generated for/);
});

test('a broken pointer warns instead of being read as unlinked', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  fs.writeFileSync(path.join(dir, 'dev/flowpad/project.json'), '{ not json');
  assert.match(line(run(dir, ['check']).out, 'project'), /WARN.*not valid JSON/);

  fs.writeFileSync(path.join(dir, 'dev/flowpad/project.json'), JSON.stringify({ projectId: 'x' }));
  assert.match(line(run(dir, ['check']).out, 'project'), /WARN.*not a valid id/);
});

test('markdown tasks reappearing under a linked repo raise a relapse alarm', () => {
  const dir = repo({ 'package.json': { name: 'app' } });
  run(dir, ['init', '--agent=claude']);
  fs.mkdirSync(path.join(dir, 'dev/tasks/claude'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'dev/tasks/claude/A-TASK.md'), '# a task\n');

  // Unlinked, this folder is simply how the project works — no row at all.
  assert.strictEqual(line(run(dir, ['check']).out, 'task-surface'), '');

  run(dir, ['link', ID]);
  const row = line(run(dir, ['check']).out, 'task-surface');
  assert.match(row, /WARN/);
  assert.match(row, /1 file\(s\)/);
  assert.match(row, /relapse/);
});
